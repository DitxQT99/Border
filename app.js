/* TARGETKU — single-page habit/goal tracker
   V4: Profile photo + frame reward system + performance optimizations.
*/
...
/* ============ FRAMES ============ */
/* Semua frame dirender via SVG inline (viewBox 0 0 100 100).
   Avatar photo duduk di dalam 72% area, frame digambar di r=38..50. */
...
const FRAMES = [
  {
    ...
    render: () => SVG(`
      <circle cx="50" cy="50" r="46.5" fill="none" stroke="rgba(200,205,225,.55)" stroke-width="1"/>
    `)
  },
  {
  ...
  },
  {
    ...
    render: () => SVG(`
      ...
      ${Array.from({length:12},(_,i)=>{
        ...
        const x2 = 50+Math.sin(a)*47, y2 = 50-Math.cos(a)*47;
        return `<line x1="${x1.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x2.toFixed(1)}" y2="${y2.toFixed(1)}" stroke="rgba(255,202,103,.65)" stroke-width="0.5" stroke-linecap="round"/>`;
      }).join("")}
    `)
  },
  {
    ...
    render: () => SVG(`
      <defs>
        <linearGradient id="auroraGrad" x1="0" y1="0" x2="1" y2="1">
          ...
          <stop offset=".5" stop-color="#9b8cff"/>
    ...
    render: () => SVG(`
      <defs>
        <linearGradient id="phoenixGrad" x1="0" y1="0" x2="1" y2="1">
          ...
          <stop offset="1" stop-color="#ff6878"/>
        </linearGradient>
      </defs>
  ...
  },
  {
    ...
    render: () => SVG(`
      <defs>
        <linearGradient id="eclGrad" x1="0" y1="0" x2="1" y2="0">
    ...
    render: () => SVG(`
      <defs>
        <linearGradient id="crownGrad" x1="0" y1="0" x2="1" y2="0">
          ...
          <stop offset=".5" stop-color="#ff9d5c"/>
    ...
    render: () => SVG(`
      <defs>
        <linearGradient id="relicGrad" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stop-color="#9b8cff"/>
          <stop offset=".5" stop-color="#ffca67"/>
...
let statsCache = { stamp:null, week:0, month:0, avg7:0, category:[], insight:"" };
...
function bindGlobal(){
  ...
  $("#rewardView").addEventListener("click", ()=>{ const f=currentRewardFrame; closeReward(); processRewardQueue(); if(f){ navigate("frames"); } });
  $("#photoInput").addEventListener("change", handlePhotoPick);
  $("#importFile").addEventListener("change", handleImportFileChange);
  window.addEventListener("keydown",(e)=>{ if(e.key==="Escape"){ closeTargetModal(); closeConfirm(); closeInputModal(); closeCropper(); closeReward(); } });
}
...
function toggleTarget(k, id){
  ...
  if(k < today){ toast("Tanggal lampau terkunci — tidak bisa diubah."); return; }
  if(k > today){ toast("Belum waktunya. Fokus hari ini dulu ✦"); return; }
  const day = ensureDay(k);
  if(day.done[id]){ toast("Sudah selesai ✓ — terkunci."); return; }
  day.done[id] = {at: Date.now()};
  haptic();

  // Selective DOM update for home / focus / calendar without full render
  ...
  // Recalculate derived (streak, xp, achievements, frames)
  updateDerived();
  // Update visible numbers
  if(route==="home" && k===today) updateHomeNumbers(k);
  else if(route==="calendar") updateCalendarNumbers();
  // Full re-render only on Focus page (remaining list shrinks)
  if(route!=="home" && route!=="calendar" && route!=="frames" && route!=="profile") render();
...
function saveCropper(){
  ...
  const finalScale = coverScale * userScale * k;
  ...
  canvas.toBlob(async (blob)=>{
    ...
    closeCropper();
    await hydratePhotos();
    if(route==="profile"||route==="frames") render();
    else render();
  ...
  return `
    ...
    <section class="section card cv-auto" style="padding:16px">
      ...
      <div class="milestones">${[3,7,14,30,60,100].map(m=>{
        const key = "streak_"+m;
        const on = state.achievements[key] || state.achievements[m];
...
function renderProfile(){
  ...
  return `
    <div class="page-head"><div><div class="eyebrow">ACCOUNT</div><h1>Profil</h1><p>Pengaturan dan koleksi reward.</p></div></div>
    ...
    <section class="card profile-card">
      <div class="profile-header">
        ...
        <div>
          ...
          <p class="profile-goal">${escapeHTML(state.user.goal||"Membangun rutinitas yang konsisten")}</p>
          <div style="display:flex;justify-content:center;margin-top:8px">
            <span class="profile-frame-label">✦ Frame: <b>${escapeHTML(frame.name)}</b></span>
          </div>
        </div>
        <div class="profile-actions">
          <button class="btn ghost" id="btnChangePhoto">Ganti Foto</button>
          <button class="btn primary" data-action="frames">Koleksi Frame</button>
        </div>
      </div>
      <div class="profile-stats">
        ...
        <div class="metric"><b>Lv.${state.xp.level}</b><span>level</span></div>
      </div>
    </section>
  ...
  return `
    ...
    <section class="frame-preview-card">
      ...
      <div class="frame-preview-meta">
        ...
        <span>🔥 ${state.streak.current} hari streak</span>
      </div>
      <div class="frame-preview-tier">${tierBadge(equipped.tier)}</div>
    </section>
    ...
    <section class="section">
      <div class="frames-head">
        ...
        <span class="count">${unlockedCount} / ${FRAMES.length} Terbuka</span>
      </div>
      <div class="frames-grid">
        ${FRAMES.map(f=>renderFrameCard(f)).join("")}
      </div>
    </section>`;
}
function tierBadge(tier){
  const label = tier.toUpperCase();
  return `<span class="fc-tier tier-${tier}">${label}</span>`;
}
function renderFrameCard(f){
  const unlocked = !!state.unlockedFrames[f.id];
  const equipped = state.equippedFrame === f.id;
  const name = state.user.name||"Teman";
  const reqLabel = f.requirement.label;
  return `<div class="frame-card ${equipped?"equipped":""} ${unlocked?"":"locked"}" data-frame="${f.id}">
    <div class="reward-avatar">
    ...
    ${unlocked
      ...
      : `<div class="fc-status">🔒 TERKUNCI</div>
         <div class="fc-req">${escapeHTML(reqLabel)}</div>`}
  </div>`;
}

/* ============ BIND VIEW (event delegation for actions) ============ */
function bindView(){
  const view = $("#view");
...
function onViewChange(e){
  if(e.target.id==="notifSwitch"){
    if(e.target.checked){ state.settings.notifications = true; requestNotifications(); }
...
function confirmDeleteTarget(id){
  const t = state.targets.find(x=>x.id===id); if(!t) return;
  askConfirm("Hapus target?", `“${t.title}” akan dihapus. Riwayat lama tetap tersimpan.`, ()=>{
...
function openFocus(){
  ...
  $("#completeAllFocus").addEventListener("click", ()=>{
    ...
    askConfirm("Selesaikan semua?", `Ini akan menandai ${rem.length} target sebagai selesai hari ini.`, ()=>{
...
function requestNotifications(){
  if(!("Notification" in window)){ toast("Browser belum mendukung notifikasi."); return; }
  Notification.requestPermission().then(permission=>{
    state.settings.notifications = permission==="granted";
    persist(); updateNotifyButton();
    toast(permission==="granted"?"Notifikasi aktif ✓":"Izin notifikasi belum diberikan.");
  });
}
function updateNotifyButton(){
  const b = $("#notifyBtn"); if(!b) return;
  if(!("Notification" in window)){ b.textContent="🔕"; b.title="Browser tidak mendukung"; return; }
  const ok = Notification.permission==="granted" && state.settings.notifications;
  b.textContent = ok ? "🔔" : "🔕";
  b.title = ok ? "Notifikasi aktif" : "Aktifkan notifikasi";
}
function tickReminders(){
  if(!state.settings.notifications) return;
  if(!("Notification" in window) || Notification.permission!=="granted") return;
  const now = new Date(), today = todayKey();
  const minute = `${today}_${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}`;
  state.targets.filter(t=>t.active!==false && t.time && t.reminder!=="off" && targetScheduledOn(t, now)).forEach(t=>{
    ...
    if(total===cur && !isDone(today,t.id) && !notifiedMinuteKeys.has(minute+"_"+t.id)){
...
function confirmUnlockToday(){
  ...
  askConfirm("Buka Kunci Hari Ini?", `Ini menghapus ${p.done} centang hari ini. Gunakan hanya jika salah tekan.`, ()=>{
  ...
  timerInterval = setInterval(()=>{
    ...
    if(timerSeconds<=0){
      ...
      if("Notification" in window && Notification.permission==="granted")
        new Notification("TARGETKU", {body: timerMode==="Istirahat" ? "Waktunya istirahat 5 menit." : "Waktunya kembali fokus."});
